...
use LINE\Clients\Liff\FormDataProcessor;
...
class LiffApi
{
     ...
     *
     * @throws \LINE\Clients\Liff\ApiException on non-2xx response or if the response body is not in the expected format
    ...
    public function addLIFFAppRequest($addLiffAppRequest, string $contentType = self::contentTypes['addLIFFApp'][0])
    {
        ...
        if (isset($addLiffAppRequest)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                ...
                $httpBody = json_encode(ObjectSerializer::sanitizeForSerialization($addLiffAppRequest), JSON_THROW_ON_ERROR);
            } else {
    ...
    public function updateLIFFAppRequest($liffId, $updateLiffAppRequest, string $contentType = self::contentTypes['updateLIFFApp'][0])
    {
        ...
        if (isset($updateLiffAppRequest)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = json_encode(ObjectSerializer::sanitizeForSerialization($updateLiffAppRequest), JSON_THROW_ON_ERROR);
            } else {
